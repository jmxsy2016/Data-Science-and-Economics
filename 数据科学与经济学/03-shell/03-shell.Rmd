...
☑ Have you cloned the [course lecture repo](https://github.com/uo-ec510-2020-spring/lectures) to your local machine?
...
**Note:** Mac users must install `rename` from Homebrew (`$ brew rename`). This [Mac version](http://plasmasturm.org/code/rename/) also requires slightly different syntax; you'll need to include an `-s` ("substitute") flag in your commands here, e.g. `$ rename -s csv TXT meals/monday.csv`
...
For example, let's change _all_ of the file extensions in the `meals` directory.
...
sed -i 's/Bill/Jack/g' nursery.txt
```
...
PS &mdash; You can also use double quotes (") instead of single ones (') for `sed` and `awk` commands. This can sometimes run you into trouble with special symbols or patterns in the text, though. (Always be mindful of accidental matching or closing quotation mark pairs.)
...
rm -f meals/mealplan.csv ## Silently remove to avoid errors with later re-knits
```
...
We have already seen how to append text lines to a file, but when it comes to larger jobs or more complicated editing work, you're better off using [nano](https://www.nano-editor.org/). 
...
- Environment variables, SSH, memory management (e.g. [top](https://ss64.com/bash/top.html) and [htop](https://hisham.hm/htop/)), automating tasks (e.g. [cron jobs](https://www.tecmint.com/create-and-manage-cron-jobs-on-linux/)), GNU parallel, etc.
...
- The way [this works](https://www.howtogeek.com/261383/how-to-access-your-ubuntu-bash-files-in-windows-and-your-windows-system-drive-in-bash/) is that Windows drives are mounted on your WSL's `mnt` directory.
...
pagedown::chrome_print("../03-shell.html", output = "../03-shell.pdf")
```
